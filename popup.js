...
async function load() { const state = await call({ type: "status" }); const tested = state.lastConnectionTest; const configured = state.configured; const youtubeSearch = state.youtubeSearch; const searchLimited = Boolean(youtubeSearch?.rateLimited); $("state").textContent = searchLimited ? "YouTube search temporarily limited" : (configured ? "Local tunnel connection" : "Setup required"); $("tunnel-status").textContent = shortTunnel(state.tunnelId); $("tunnel-status").className = configured ? "good" : "bad"; $("youtube-status").textContent = searchLimited ? `Search paused — retry in ${youtubeSearch.retryAfterSeconds}s` : "Ready"; $("youtube-status").className = searchLimited ? "warn" : "good"; $("test-status").textContent = tested ? (tested.success ? "Successful" : "Failed") : "Not run"; $("test-status").className = tested?.success ? "good" : (tested ? "bad" : ""); }
...
$("chatgpt").addEventListener("click", () => call({ type: "open-external", target: "chatgpt" }));
$("setup").addEventListener("click", () => chrome.tabs.create({ url: chrome.runtime.getURL("onboarding.html"), active: true }));
