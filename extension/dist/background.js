...
var EXTENSION_VERSION = "1.31.0";
var REQUIRED_AGENT_INTERFACE_VERSION = 28;
var CAPTURE_FRAME_WIDGET_URI = "ui://researchtube/capture-frame-v29.html";
...
var workspaceShareStatusSchema = {
  ...
  properties: {
    ...
    publicBaseUrl: { ...nullableString, pattern: "^https://", description: "Temporary URL for an external browser or HTTP client to download allowed files. ChatGPT may be unable to fetch a Quick Tunnel URL or use it as visual input, so do not rely on it for image inspection." },
...
var debugBannerConfigurationSchema = {
  type: "string",
  enum: ["banner_suppressed", "banner_enabled", "mixed", "unknown"]
};
var debugBannerResultSchema = {
  type: "object",
  additionalProperties: false,
  properties: {
...
var CDP_CLICK_SEND_BUTTON_EXPRESSION = `(() => {
...
async function cdpSendAttachedImages(tabId, fileCount) {
  ...
  await cdpWaitFor(tabId, CDP_ENABLED_SEND_BUTTON_EXPRESSION, "the enabled ChatGPT Send button", 9e4);
...
function normalizeDebugBannerAgentResult(value) {
  ...
  if (!value || typeof value !== "object" || !["banner_suppressed", "banner_enabled", "mixed", "unknown"].includes(configuration) || !(typeof chromeRunning === "boolean" || chromeRunning === null) || !Number.isInteger(browserInstances) || browserInstances < 0 || requiredSwitch !== "--silent-debugger-extension-api" || typeof message !== "string" || !message.trim()) {
...
function normalizeCaptureFrameInput(argumentsValue = {}) {
  const args = captureFrameObject(argumentsValue, "capture_frame", /* @__PURE__ */ new Set(["path", "youtube", "timestampSeconds", "videoStreamIndex", "seekMode", "applyDisplayRotation", "crop", "resize", "image", "outputPath", "showInChat"]));
  if (args.path === void 0 === (args.youtube === void 0)) throw localAgentError("CAPTURE_FRAME_INVALID", "capture_frame requires exactly one source: path or youtube.");
...
function normalizeScreenCaptureInput(argumentsValue = {}) {
  const args = captureFrameObject(argumentsValue, "capture_screen", /* @__PURE__ */ new Set(["outputPath", "image", "showInChat"]));
...
function normalizeImageCropInput(argumentsValue = {}) {
  const args = captureFrameObject(argumentsValue, "researchtube_image_crop", /* @__PURE__ */ new Set(["path", "crop", "image", "outputPath", "showInChat"]));
