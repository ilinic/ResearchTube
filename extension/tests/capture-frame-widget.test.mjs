...
const widget = await readFile(new URL("../ui/capture-frame-widget-v21.html", import.meta.url), "utf8");
...
assert.match(widget, /id="library-retry"/);
assert.match(widget, /Retry file upload/);
assert.match(widget, /id="analyze-frame"/);
assert.match(widget, /Ask ChatGPT to analyze frame/);
assert.match(widget, /save requested/);
assert.match(widget, /uploadFile available/);
assert.match(widget, /upload failed/);
assert.match(widget, /user retry/);
assert.match(widget, /safeLibraryError/);
assert.match(widget, /sendFollowUpMessage\(\{ prompt:/);
assert.match(widget, /vision input staged/);
assert.match(widget, /requestedLibrarySave/);
assert.match(widget, /toolInput\?\.delivery/);
assert.match(widget, /uploaded\?\.id/);
assert.match(widget, /Array\.isArray\(current\.imageIds\)/);
assert.match(widget, /\{ \.\.\.current, modelContent, privateContent, imageIds \}/);
