...
assert.match(background, /const REQUIRED_AGENT_INTERFACE_VERSION = 66;/);
...
assert.match(background, /records only its tab audio as WebM\/Opus/, "Google Translate tab audio must be retained as WebM\/Opus");
...
assert.match(background, /if \(input\.outputMode === "speakers"\) \{[\s\S]*?googleTranslatePressListen\(tab\.id, controller\.signal, true\);[\s\S]*?google-translate-complete/, "speaker-only Google TTS must click Listen without requiring tab-audio capture");
