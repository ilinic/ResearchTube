...
- `researchtube_image_crop(path, crop, …)` cuts a rectangle from one existing PNG, JPEG, or WebP workspace image. Coordinates are stored-image pixels, the source remains unchanged, and the Agent verifies that the requested rectangle lies within it before saving a distinct PNG, JPEG, or WebP workspace image.
- `capture_screen` captures one desktop image, not a recording. FFmpeg is the sole pixel-capture implementation: `gdigrab` on Windows, `x11grab` on Linux/X11, and `avfoundation` on macOS. Platform display queries provide virtual-desktop geometry and monitor count only.
- `capture_frame`, `capture_screen`, and `researchtube_image_crop` accept `showInChat` (default `false`). When true, their one MCP result opens a context-specific workspace-image widget; when false, the widget has zero height and the workspace file is still created normally. `researchtube_show_workspace_image(path)` displays any existing workspace PNG, JPEG, or WebP later. The widget preserves YouTube links only for frame/crop filenames with valid provenance and never links a local timestamp from a `[partial_…]` file to YouTube.
