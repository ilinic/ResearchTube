...
- `tools/list` returns the YouTube research tools, Local Agent status, Local Agent download tools, sandboxed workspace tools, `media_probe`, image tools, and the explicit Windows clipboard tools `clipboard_status`, `clipboard_get`, and `clipboard_set`.
...
- `media_image_inspect(path)` independently validates a PNG, JPEG, or WebP workspace file and returns its verified format, MIME type, dimensions, and byte size. It never returns image bytes or a host path.
...
- `media_capture_frame(path, timestampSeconds, …)` extracts one selected video frame with locally resolved `ffmpeg` and `ffprobe`. `accurate` seeking decodes to the first frame with a presentation timestamp at or after the requested time and returns that actual timestamp; `fast` seeking is approximate and reports no false timestamp. It can apply display rotation, crop, resize with `contain`/`cover`/`stretch`, including upscale, and encode PNG, JPEG, or WebP. The image is always stored at an explicit or generated logical workspace path; metadata reports its actual encoded dimensions and byte size, but never a host path.
- `media_image_crop(path, crop, …)` cuts a rectangle from one existing PNG, JPEG, or WebP workspace image. Coordinates are stored-image pixels, the source remains unchanged, and the Agent verifies that the requested rectangle lies within it before saving a distinct PNG, JPEG, or WebP workspace image.
- `media_capture_screen` captures one desktop image, not a recording. Without `region`, it captures the complete virtual desktop; with `region: {x, y, width, height}`, it captures that fully bounded global desktop rectangle. FFmpeg is the sole pixel-capture implementation: `gdigrab` on Windows and `x11grab` on Linux/X11 receive the rectangle directly; macOS uses FFmpeg's crop filter after the AVFoundation display composition. Platform display queries provide virtual-desktop geometry and monitor count only. Auto-named regional captures include `_x…_y…_w…_h…` immediately before their random ID.
- `camera_list`, `camera_capture_frame`, `camera_record_video`, `camera_record_audio`, `camera_record_status`, and `camera_record_stop` use FFmpeg's native camera inputs: DirectShow on Windows, V4L2 on Linux, and AVFoundation on macOS. Camera IDs are short opaque Agent-lifetime handles; native device identities and paths never cross the Agent boundary. Frames are stored directly under `captures/`. Every automatic filename is `<camera name> <YYYY-MM-DDTHH_MM_SSZ> [cam_<task-id>]` with the output extension; it is Windows-safe and uses underscores instead of time colons. `camera_list` advertises the largest native resolution for every available FPS strictly above 25 and no greater than 120, preserving native rates such as 29.97 and 59.94. Video recording accepts an optional FPS and uses the largest native resolution close to it; without one, it prefers a rate near 60 FPS and then one near 30 FPS. `camera_record_video` is asynchronous H.264 MP4 with the paired camera microphone and a maximum duration of 60 seconds; its final dimensions, FPS, duration, and audio-track state are verified with ffprobe. `camera_record_audio` is asynchronous AAC/M4A from that microphone with a maximum duration of 10 minutes. Both use the same status/stop task lifecycle; videos are stored under `webcamera/`, audio under `sound/`.
- `media_capture_frame`, `media_capture_screen`, and `media_image_crop` have the same `showInChat` contract (default `false`). When true, the caller follows the successful creation result with exactly one `media_image_show(path)` call; when false, it does not create a display widget. `media_image_show(path)` displays supported workspace images, videos, and audio files. The widget preserves a canonical `[yt_<11-character-id>]` link for any matching workspace filename, never links a local timestamp from a `[partial_…]` file to YouTube, and recognizes a strict `[cam_<10-character-task-id>]` tag as webcam provenance.
- The workspace-media widget always receives a direct loopback URL whose path after the port is the logical workspace path, so the browser loads media bytes from the Local Agent. Media bytes are never placed in MCP metadata and this route never starts a public tunnel.
...
### User-selected MCP tools

Extension Settings stores availability by exact public MCP tool name. A removed name is retained harmlessly, a reintroduced identical name regains its prior state, and a renamed tool is treated as new. Newly encountered names receive the user's `newToolsEnabledByDefault` setting, initially enabled. Tool group is explicit metadata; an unclassified extension tool appears in `Custom`. `system_agent_status` is the always-enabled bootstrap tool. Private widget-only actions are never listed or filtered. Settings changes apply to the next `tools/list`; the user refreshes the schema through ChatGPT Plugins → ResearchTube → Manage → **Refresh**. A stale schema calling a disabled public tool receives the structured `TOOL_DISABLED` result.
...
├── settings.html / settings.js     # single local Settings UI
