...
- `media_camera_list`, `media_camera_capture_frame`, and `media_camera_record_*` use FFmpeg's native camera inputs: DirectShow on Windows, V4L2 on Linux, and AVFoundation on macOS. Camera IDs are opaque Agent-lifetime handles; native device identities and paths never cross the Agent boundary. Frames are stored under `captures/camera/`. `media_camera_list` advertises the largest available native mode for each 30 or 60 FPS recording rate and whether a matched microphone is available. `media_camera_record_video` writes H.264 MP4 with matched-microphone AAC audio under `webcamera/` for up to 60 seconds; `media_camera_record_audio` writes AAC/M4A under `sound/` for up to 10 minutes. Both are asynchronous and use the shared status/stop tools.
- `media_capture_frame`, `media_capture_screen`, and `media_image_crop` have the same `showInChat` contract (default `false`). When true, the caller follows the successful creation result with exactly one `media_show_in_chat(path)` call; when false, it does not create a display widget. `media_show_in_chat(path)` displays workspace images and recorded audio. The widget preserves a canonical `[yt_<11-character-id>]` link for any matching workspace filename and never links a local timestamp from a `[partial_…]` file to YouTube.
- The workspace-image widget always receives a direct loopback URL whose path after the port is the logical workspace path, so the browser loads image bytes from the Local Agent. Image bytes are never placed in MCP metadata and this route never starts a public tunnel.
