...
- `media_capture_screen` captures one desktop image, not a recording. FFmpeg is the sole pixel-capture implementation: `gdigrab` on Windows, `x11grab` on Linux/X11, and `avfoundation` on macOS. Platform display queries provide virtual-desktop geometry and monitor count only.
