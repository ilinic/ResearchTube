...
- `media_inspect_image(path)` independently validates a PNG, JPEG, or WebP workspace file and returns its verified format, MIME type, dimensions, and byte size. It never returns image bytes or a host path.
...
- `media_capture_frame`, `media_capture_screen`, and `media_image_crop` have the same `showInChat` contract (default `false`). When true, the caller follows the successful creation result with exactly one `media_show_workspace_image(path)` call; when false, it does not create a display widget. `media_show_workspace_image(path)` also displays any existing workspace PNG, JPEG, or WebP later. The widget preserves a canonical `[yt_<11-character-id>]` link for any matching workspace filename and never links a local timestamp from a `[partial_…]` file to YouTube.
