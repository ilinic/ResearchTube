    ...
    const [{ result }] = await chrome.scripting.executeScript({
      ...
      func: () => {
        const visible = (element) => {
          const style = getComputedStyle(element);
          const rect = element.getBoundingClientRect();
          return !element.disabled && style.visibility !== "hidden" && style.display !== "none" && rect.width > 2 && rect.height > 2;
        };
...
async function startGoogleTranslateSpeechTask(taskId2, uploadToken, input) {
  ...
  try {
    ...
    await googleTranslateProgress(taskId2, uploadToken, "playing", 20);
    ...
    await googleTranslateOffscreen({ type: "researchtube_google_translate_start", taskId: taskId2, streamId, record: input.outputMode !== "speakers", relayAudio: input.outputMode !== "file" });
    await googleTranslateProgress(taskId2, uploadToken, input.outputMode === "speakers" ? "playing" : "recording", 35);
    ...
    await googleTranslateOffscreen({ type: "researchtube_google_translate_finish", taskId: taskId2, uploadUrl: input.outputMode === "speakers" ? null : uploadUrl });
