...
const MCP_TOOL_SETTINGS = Object.freeze({
  ...
  media_probe: { group: "media" }, media_capture_frame: { group: "media" }, media_capture_frame_get_task: { group: "media" }, media_capture_frame_task_diagnostics: { group: "media" }, media_capture_frame_cancel_task: { group: "media" }, media_capture_screen: { group: "media" }, media_image_crop: { group: "media" }, media_image_show: { group: "media" }, media_image_inspect: { group: "media" },
...
function toolDefinitions() {
  return [
    ...
    },
    {
      ...
      title: "Cancel speech",
      description: "Stop a working speech task as quickly as practical. Cancellation stops playback or recording, closes an automation-created Google Translate tab, releases local resources, and is a normal terminal outcome.",
      annotations: localWorkspaceWriteAnnotations,
      inputSchema: { type: "object", additionalProperties: false, properties: { taskId: { type: "string", minLength: 1 } }, required: ["taskId"] },
    ...
    },
    {
      ...
      annotations: localWorkspaceWriteAnnotations,
      inputSchema: { type: "object", additionalProperties: false, properties: { cameraId: { type: "string", minLength: 1 }, durationSeconds: { type: "integer", minimum: 1, maximum: 60 }, targetFps: { type: "number", exclusiveMinimum: 25, maximum: 120 } }, required: ["cameraId", "durationSeconds"] }, outputSchema: cameraRecordTaskSchema
    },
    {
...
async function googleTranslatePressListen(tabId, signal) {
  const deadline = Date.now() + GOOGLE_TRANSLATE_TAB_TIMEOUT_MS;
  while (Date.now() < deadline) {
    googleTranslateAbort(signal);
    const [{ result }] = await chrome.scripting.executeScript({
      target: { tabId },
      func: () => {
        const visible = element => { const style = getComputedStyle(element); const rect = element.getBoundingClientRect(); return !element.disabled && style.visibility !== "hidden" && style.display !== "none" && rect.width > 2 && rect.height > 2; };
        const label = element => [element.getAttribute("aria-label"), element.getAttribute("data-tooltip"), element.getAttribute("title"), element.textContent].filter(Boolean).join(" ").toLowerCase();
        const controls = [...document.querySelectorAll("button, [role='button']")].filter(visible);
        const sourceControl = controls.find(element => {
          const value = label(element);
          return /listen|прослушать|écouter|anhören|ascolta|escuchar/.test(value) && !/translation|перевод|traduction|übersetzung|traduzione|traducción/.test(value);
        });
  ...
  throw new Error("Google Translate listen control is unavailable.");
}
...
async function startGoogleTranslateSpeechTask(taskId, uploadToken, input) {
  ...
  try {
    ...
    await googleTranslateProgress(taskId, uploadToken, "playing", 20);
    ...
    await googleTranslateOffscreen({ type: "researchtube_google_translate_start", taskId, streamId, record: input.outputMode !== "speakers", relayAudio: input.outputMode !== "file" });
    await googleTranslateProgress(taskId, uploadToken, input.outputMode === "speakers" ? "playing" : "recording", 35);
    await googleTranslatePressListen(tab.id, controller.signal);
    const config = await getConfig();
    ...
    await googleTranslateOffscreen({ type: "researchtube_google_translate_finish", taskId, uploadUrl: input.outputMode === "speakers" ? null : uploadUrl });
