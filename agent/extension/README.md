...
Iteration 1 also includes an optional standard-library Python Local Agent in the release's `agent/` folder. Run `python researchtube_agent.py` there to start its private `127.0.0.1:17843` health service, then select **Test connection** in the Local Agent section of Settings. It creates a local `workspace/` and reports optional `yt-dlp`, `ffmpeg`, and `ffprobe` availability. It has no download, Task, or media-processing capability yet, and all YouTube tools work unchanged while it is stopped.
...
- The optional Local Agent is a loopback-only health service. It has no credentials, accepts no media work in Iteration 1, and exposes only `GET /health` to the extension.
