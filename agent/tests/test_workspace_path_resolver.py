...
class CaptureFrameTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_speech_options_and_task_snapshot_are_compact(self) -> None:
        ...
        self.assertRegex(agent.speech_default_workspace_path("Google Translate (auto)", "tts_AbCdEf1234", "mp3"), r"^text-to-speech/Google Translate \(auto\) \d{4}-\d{2}-\d{2}T\d{2}_\d{2}_\d{2}Z \[tts_AbCdEf1234\]\.mp3$")
        ...
        google_task = agent.SpeechTask(task_id="tsk_google123", text="Привет", voice_id=None, voice_name="Google Translate (auto)", output_mode="file", output_path="text-to-speech/Google Translate (auto) [tts_test].mp3", created_at="2026-01-01T00:00:00Z", last_updated_at="2026-01-01T00:00:00Z", engine="googleTranslate", upload_token="x" * 24)
        ...
        self.assertTrue(created["outputPath"].endswith(".mp3"))
        ...
        uploaded = asyncio.run(google_manager.google_audio(created["taskId"], created["uploadToken"], b"\xff\xfb" + b"mp3" * 40))
        self.assertEqual(uploaded["result"]["mimeType"], "audio/mpeg")
        self.assertEqual((agent.WORKSPACE_PATH / uploaded["outputPath"]).read_bytes()[:2], b"\xff\xfb")
